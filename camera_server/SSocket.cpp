...
bool SSocket::valid() {
    return (this->res == CURLE_OK) ? true : false;
}
bool SSocket::check(CURLcode newRes) {
    return ((this->res = newRes) == CURLE_OK ? true : false);
}
const char * SSocket::error() {
    std::string errorBuffer("[Error] ");
    errorBuffer += curl_easy_strerror(this->res);
...
std::string SSocket::execute() {
    ...
    if (! this->curl)
        return "[Error] Curl initialization failure";
...
char *SSocket::readBytesfromFile(const std::string filename, int& size)
{
	std::ifstream fl(filename.c_str(), std::ios::binary);
...
std::string SSocket::postUpload(const std::string & url, const std::string &filename, const std::string &key) {
    ...
    if (this->curl) {
        ...
        int size = 0;
        char *contents = readBytesfromFile(filename, size);
        curl_formadd(&post,
            &last,
        ...
        std::string buffer = filename.substr(filename.rfind("\\") + 1);

        curl_formadd(&post, &last,
            CURLFORM_COPYNAME, key.c_str(),
            CURLFORM_BUFFER, buffer.c_str(),
            CURLFORM_BUFFERPTR, contents,
            CURLFORM_BUFFERLENGTH, size,
            CURLFORM_END);

        this->option(CURLOPT_HTTPPOST, post);
